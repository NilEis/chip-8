...
#ifndef __EMSCRIPTEN__
    pthread_create (
        &(chip_8.sound_timer.timer.thread), NULL, cpu_sound_timer, NULL);
#endif
}
...
uint64_t cpu_tick (uint64_t *execution_time)
{
    ...
    case 0x4:
        ...
        break;
    case 0x5:
        if (chip_8.regs[(fetched & 0x0F00) >> 8]
            == chip_8.regs[(fetched & 0x00F0) >> 4])
        {
            chip_8.pc += 2;
        }
        break;
    ...
    case 0xd:
    {
        ...
        for (int y_i = 0; y_i < n; y_i++)
        {
            for (int x_i = 0; x_i < 8; x_i++)
            {
                ...
                chip_8.screen.data[index]
                    ^= (chip_8.mem[chip_8.i + y_i] >> (7 - x_i)) & 0x0001;
...
static void cpu_disasm (uint16_t pc)
{
    ...
    printf ("pc[%03" PRIX16 "] - %02" PRIX8 "%02" PRIX8 ": ",
        pc,
        code[0],
        code[1]);
    switch (code[0] >> 4)
    ...
    break;
    case 0x01:
        printf ("JMP  0x%" PRIX16 "%02" PRIX16
                "                   // Jump to address\n",
            code[0] & 0x0F,
            code[1]);
        break;
    case 0x02:
        printf ("CALL 0x%" PRIX16 "%02" PRIX16
                "                   // Call subroutine\n",
            code[0] & 0x0F,
            code[1]);
        break;
    case 0x03:
        printf ("IFEQ V%" PRIX8 " 0x%02" PRIX16
                "                 // Skip next instruction if V%" PRIX8
                " EQ 0x%02" PRIX8 "\n",
            code[0] & 0x0F,
            code[1],
            code[0] & 0x0F,
        ...
        break;
    case 0x04:
        printf ("IFNQ V%" PRIX8 " 0x%02" PRIX16
                "                 // Skip next instruction if V%" PRIX8
                " NOT EQ 0x%02" PRIX8 "\n",
            code[0] & 0x0F,
            code[1],
    ...
    case 0x06:
    {
        printf ("MOV  V%" PRIX8 ", 0x%" PRIX16
                "                 // Set V%" PRIX8 " to 0x%" PRIX8 "\n",
            code[0] & 0x0F,
            code[1],
            code[0] & 0x0F,
            code[1]);
    ...
    break;
    case 0x07:
        printf ("ADD. V%" PRIX8 ", 0x%" PRIX16
                "                 // Set V%" PRIX8 " to V%" PRIX8
                " + 0x%" PRIX8 "\n",
            code[0] & 0x0F,
            code[1],
            code[0] & 0x0F,
    ...
    case 0x08:
        ...
        case 0x3:
            ...
            break;
        case 0x4:
            printf ("ADD. V%" PRIX8 ", V%" PRIX8
                    "                   // Set V%" PRIX8 " to V%" PRIX8
                    " + V%" PRIX8 " and set VF to 1 if overflow\n",
                code[0] & 0x0F,
                code[1] >> 4,
                code[0] & 0x0F,
                code[0] & 0x0F,
            ...
            break;
        case 0x5:
            printf ("SUB. V%" PRIX8 ", V%" PRIX8
                    "                   // Set V%" PRIX8 " to V%" PRIX8
                    " - V%" PRIX8 " and set VF to 1 if underflow(?)\n",
                code[0] & 0x0F,
                code[1] >> 4,
                code[0] & 0x0F,
                code[0] & 0x0F,
            ...
            break;
        case 0x6:
            printf ("SHR. V%" PRIX8 "                       // Shift V%" PRIX8
                    " one to the right and move the dropped bit to VF\n",
                code[0] & 0x0F,
                code[0] & 0x0F);
            break;
        case 0x7:
            printf ("SBB. V%" PRIX8 ", V%" PRIX8
                    "                   // Set V%" PRIX8 " to V%" PRIX8
                    " - V%" PRIX8 " and set VF to 1 if underflow(?)\n",
                code[0] & 0x0F,
                code[1] >> 4,
            ...
            break;
        case 0xE:
            printf ("SHL. V%" PRIX8 "                       // Shift V%" PRIX8
                    " one to the left and move the dropped bit to VF\n",
                code[0] & 0x0F,
                code[0] & 0x0F);
            break;
        default:
    ...
    case 0x0a:
    {
        printf ("SETI 0x%" PRIX8 "%02" PRIX8
                "                   // Set I to 0x%" PRIX8 "%02" PRIX8 "\n",
            code[0] & 0x0F,
            code[1],
            code[0] & 0x0F,
            code[1]);
    ...
    case 0x0c:
        ...
        break;
    case 0x0d:
        printf ("DRAW V%" PRIX8 "V%" PRIX8 " N%" PRIX8
                "                 // Draw the sprite in memory with %" PRIX8
                " rows at I on coordinates V%" PRIX8 ", V%" PRIX8 "\n",
            code[0] & 0x0F,
            code[1] >> 4,
            code[1] & 0x0F,
            code[1] & 0x0F,
...
static void *cpu_delay_timer (void *args)
{
    ...
    {
        pthread_mutex_lock (&(chip_8.delay_timer.lock));
        chip_8.delay_timer.t
            = (chip_8.delay_timer.t != 0) * (chip_8.delay_timer.t - 1);
        pthread_mutex_unlock (&(chip_8.delay_timer.lock));
        SDL_Delay (16);
    }
    return NULL;
...
static void *cpu_sound_timer (void *args)
{
    ...
    {
        pthread_mutex_lock (&(chip_8.sound_timer.timer.lock));
        chip_8.sound_timer.timer.t = (chip_8.sound_timer.timer.t != 0)
                                   * (chip_8.sound_timer.timer.t - 1);
...
bool cpu_stopped (void) { return chip_8.should_stop; }
